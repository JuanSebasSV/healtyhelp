    ...
    if (password) {
      if (user.googleId && !user.password) {
        return res.status(400).json({
          error: "Las cuentas de Google no pueden cambiar la contraseña",
        });
      }
...
router.delete("/avatar", protect, async (req, res) => {
  try {
    ...
    res.json({
      ...
      user: { ...user.toObject(), avatar: null },
    });
  } catch (error) {
...
router.delete("/account", protect, async (req, res) => {
  try {
    ...
    if (user.isSuperAdmin) {
      return res
        .status(403)
        .json({ error: "La cuenta Super Administrador no puede eliminarse" });
    }
  ...
  try {
    ...
    if (autoLogoutMinutes !== undefined) {
      ...
      if (isNaN(mins) || mins < 1 || mins > 480) {
        return res
          .status(400)
          .json({ error: "Los minutos deben estar entre 1 y 480" });
      }
    ...
    if (Object.keys(update).length === 0) {
      return res
        .status(400)
        .json({ error: "No se enviaron preferencias válidas" });
    }
