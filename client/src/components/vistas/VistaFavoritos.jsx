...
const VistaFavoritos = ({ recetas, favoritos, toggleFav }) => {
  const navigate = useNavigate();
  const [categoriaActiva, setCategoriaActiva] = useState(null); // null = vista inicial
      ...
      <div className="vf-wrap">
        ...
        <div className="vf-header">
          ...
          <p className="vf-subtitulo">
            Selecciona una categoría para explorar tus recetas favoritas
          </p>
        </div>
  ...
  return (
    <div className="vf-wrap2">
      {/* Header con breadcrumb */}
      <div className="vf-header vf-header--inner">
        <button className="vf-back" onClick={() => { setCategoriaActiva(null); setBusqueda(''); }}>
      ...
      <span className="vf-total-badge">
            {recetasFavoritas.filter(r => r.cat === categoriaActiva).length} receta{recetasFavoritas.filter(r => r.cat === categoriaActiva).length !== 1 ? 's' : ''}
      </span>
    </div>
      ...
      {recetasFiltradas.length === 0 ? (
        <div className="vf-vacio">
          ...
          <h3>{busqueda ? 'Sin resultados' : `Sin favoritos en ${catActual.nombre}`}</h3>
          <p>
            {busqueda
