...
// La versión activa la trae la BD. Esta constante solo se usa como fallback
// cuando no hay ningún documento en BD todavía (primer despliegue).
export const TERMS_KEY = 'terms_accepted_local';

// Contenido estático de respaldo — se muestra si la BD aún no tiene términos.
const CONTENIDO_FALLBACK = `
...
<p>Estos Términos se rigen por las leyes de la República de Colombia. Para cualquier controversia, las partes se someten a la jurisdicción de los jueces y tribunales competentes de la ciudad de Bogotá D.C., Colombia, renunciando a cualquier otro fuero que pudiera corresponderles.</p>
...
const ModalTerminos = ({ onAceptar, esActualizacion = false }) => {
  ...
  const [termsData,  setTermsData]  = useState(null);   // { version, content, publishedAt }
  const [loadingTerms, setLoadingTerms] = useState(true);

  // Cargar términos desde la BD al montar
  useEffect(() => {
    const cargarTerminos = async () => {
      try {
        ...
        // Si falla la red usamos el fallback estático silenciosamente
        setTermsData({ version: '1.0.0', content: null });
      } finally {
  ...
  const contenidoHtml = termsData?.content || CONTENIDO_FALLBACK;
        ...
        <div className="terminos-footer">
          <label className="terminos-checkbox-label">
            <input
              ...
              disabled={!scrollado || loadingTerms}
            />
            <span>
          ...
          <button
            ...
            disabled={!aceptado || !scrollado || cargando || loadingTerms}
          >
