...
const Icons = {
  ...
  Spinner: memo(() => <div className="spinner-small" />),
  Chevron: memo(() => (
...
Object.keys(Icons).forEach(k => { if (Icons[k].displayName === undefined) Icons[k].displayName = k; });
...
const BlockDropdown = memo(({ value, onChange }) => {
      ...
      {open && (
        <div className="terms-block-menu">
          {BLOCK_OPTIONS.map(opt => (
            ...
            >
              <span className={`terms-block-option-label terms-block-option-label--${opt.value}`}>{opt.label}</span>
...
const TbBtn = memo(({ title, active, onClick, children }) => (
  <button
...
const TermsManager = () => {
  ...
  const [blockActual,   setBlockActual]   = useState('p');
  ...
  const syncFormatos = useCallback(() => {
    ...
    setBlockActual(['p', 'h1', 'h2', 'h3'].includes(raw) ? raw : 'p');
    setActiveFormats({
  ...
  const togglePreview = useCallback(() => {
    setPreview(v => {
      if (!v && previewRef.current && editorRef.current) {
    ...
    <div className="terms-manager">
      ...
      <div className="terms-manager-header">
        ...
        <div className="terms-manager-actions">
          <button className={`btn-preview${preview ? ' active' : ''}`} onClick={togglePreview} type="button">
      ...
      {preview && (
        <div className="terms-preview-container">
          ...
          <div ref={previewRef} className="terms-preview-content" />
        </div>
      )}

      <div className="terms-manager-footer">
        <button className="btn-publicar" onClick={handlePublicar} disabled={publicarDisabled} type="button">
          {publicando
