...
const Dashboard = ({ onBadgeChange }) => {
  ...
  const fetchBadge = useCallback(async () => {
    try {
      const { data } = await api.get('/admin/imagenes-resenas?estado=pendiente&solo_total=true');
  ...
  useEffect(() => {
    let activo = true;
    const poll = () => { if (activo) fetchBadge(); };
    poll();
  ...
  const { currentUsers, totalPages } = useMemo(() => {
    const last  = currentPage * USERS_PER_PAGE;
    const first = last - USERS_PER_PAGE;
    return {
      currentUsers: filteredUsers.slice(first, last),
    ...
    <div className="admin-dashboard">
      ...
      {activeTab === 'users' && (
        <>
          ...
          <UserList users={currentUsers} onDelete={handleDeleteUser} onChangeRole={handleChangeRole} />
      ...
      {activeTab === 'imagenes' && <ImagenesAprobacion onCambio={handleImagenesCambio} />}
    </div>
  );
};
