...
const ModalGooglePassword = ({ token, onSuccess }) => {
  ...
  const handleSubmit = useCallback(async () => {
    const pwdError = validarPassword(pass);
    ...
    const result = await setGooglePassword(pass, token);
      ...
      <div className="mgp-card" role="dialog" aria-modal="true" aria-labelledby="mgp-titulo">
        ...
        <div className="mgp-form">
          <div className="mgp-requisitos">
            ...
            <ul>
              <li className={pass.length >= 8 ? 'req-ok' : ''}><CheckIcon />Mínimo 8 caracteres</li>
              <li className={/[a-z]/.test(pass) ? 'req-ok' : ''}><CheckIcon />Una letra minúscula</li>
              <li className={/[A-Z]/.test(pass) ? 'req-ok' : ''}><CheckIcon />Una letra mayúscula</li>
              <li className={/\d/.test(pass) ? 'req-ok' : ''}><CheckIcon />Un número</li>
            </ul>
          </div>
          ...
          <div className="mgp-input-group">
            <input
              ...
              autoFocus
            />
            <button type="button" className="mgp-eye" onClick={() => setShowPass(p => !p)} tabIndex={-1}>
          ...
          <div className="mgp-input-group">
            <input
              ...
              disabled={loading}
            />
            <button type="button" className="mgp-eye" onClick={() => setShowConf(p => !p)} tabIndex={-1}>
