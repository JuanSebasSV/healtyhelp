...
// ✅ Fix 1: categorias y condicionesSalud estaban dentro del componente,
//    recreándose en cada render. Al ser datos constantes van fuera.
...
const VistaInicio = ({ recetas, cargandoRecetas, toggleFav, favoritos, cambiarCategoria, categoriaActiva }) => {
  ...
  // ✅ Fix 5: usar useRef importado en lugar de React.useRef
  const transitandoRef = useRef(false);
  // ✅ Fix 6: ref para controlar el intervalo y poder resetearlo
  const intervaloRef   = useRef(null);

  const navigate = useNavigate();
  const location = useLocation();

  // ✅ Fix 4: preload dentro de useEffect, no a nivel de módulo
  useEffect(() => {
  ...
  // Scrollbar verde personalizado — sin setState, directo al DOM
  useEffect(() => {
    let raf = null;
    const updateThumb = () => {
    ...
    const onScroll = () => {
      if (raf) return;
      raf = requestAnimationFrame(() => { updateThumb(); raf = null; });
    };
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e) => {
      const trackHeight = window.innerHeight * 0.6;
      const thumbHeight = trackHeight * 0.3;
      const maxThumbTop = trackHeight - thumbHeight;
      const deltaY = e.clientY - dragStartY.current;
      const deltaPct = deltaY / maxThumbTop;
      const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
      const newScroll = Math.min(Math.max(dragStartScroll.current + deltaPct * maxScroll, 0), maxScroll);
  ...
  const cambiarImagen = useCallback((indice) => {
  ...
  // ✅ Fix 6: función reutilizable para (re)iniciar el intervalo automático
  ...
  // ✅ Fix 6: al hacer click manual, el intervalo se reinicia desde 0
  //    evitando que cambie la imagen al segundo siguiente del click
  const cambiarImagenManual = useCallback((indice) => {
  ...
  useEffect(() => {
    const manejarTeclado = (e) => {
      if (e.key === 'ArrowLeft')  cambiarImagenManual(imagenActual - 1);
      if (e.key === 'ArrowRight') cambiarImagenManual(imagenActual + 1);
    };
    window.addEventListener('keydown', manejarTeclado);
    return () => window.removeEventListener('keydown', manejarTeclado);
  }, [imagenActual, cambiarImagenManual]);
  ...
  // ✅ Fix 3: ref para evitar race condition entre peticiones rápidas de filtro
  const peticionFiltroRef = useRef(null);
  ...
  const toggleFiltro = async (id) => {
    ...
    peticionFiltroRef.current = peticionActual;

    try {
      await api.put('/chat/health-profile', { condiciones: nuevos, alergias: [], preferencias: [] });
    } catch (error) {
      if (peticionFiltroRef.current === peticionActual) {
        console.error('Error guardando perfil de salud:', error);
        setFiltrosActivos(filtrosActivos); // revertir al estado anterior
      }
    }
  };

  // ✅ Fix 2: limpiarFiltros ahora también sincroniza con el backend
  const limpiarFiltros = async () => {
    setFiltrosActivos([]);
    try {
      await api.put('/chat/health-profile', { condiciones: [], alergias: [], preferencias: [] });
    } catch (error) {
      console.error('Error limpiando perfil de salud:', error);
    }
  };

  // ✅ useMemo: evita recalcular el filtro en cada render si los datos no cambiaron
  ...
  }), [recetas, categoriaActiva, filtrosActivos]);
  ...
  return (
    <div className="vistaInicio">
      ...
      <div className="hero">
        ...
        <div className="hero-dots">
          {imagenesHero.map((_, i) => (
            // ✅ Fix 6: dots usan cambiarImagenManual para resetear el intervalo
            <button key={i} className={`hero-dot ${i === imagenActual ? 'activo' : ''}`} onClick={() => cambiarImagenManual(i)} />
          ))}
        </div>
      </div>
      ...
      {/* ── Modal detalle desde notificación ── */}
      {recetaAbierta && (
        <DetalleReceta
      ...
      {/* Scrollbar verde personalizado — solo escritorio */}
