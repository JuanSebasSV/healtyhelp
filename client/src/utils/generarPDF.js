...
const wrap = (doc, txt, x, y, maxW, lh) => {
  doc.splitTextToSize(String(txt), maxW).forEach((l) => {
    doc.text(l, x, y);
    y += lh;
  });
  return y;
...
const renderReceta = (doc, receta, imgB64, idxMostrado, totalMostrado) => {
  ...
  const cat = receta.cat
    ? receta.cat.charAt(0).toUpperCase() + receta.cat.slice(1)
    : "";
  if (cat) {
    doc.setFontSize(7.5);
    doc.setFont("helvetica", "bold");
  ...
  doc.setFont("helvetica", "normal");
  rgb(doc, ...GRIS_SUAVE);
  doc.text(`Receta ${idxMostrado} de ${totalMostrado}`, W - MG, IMG_H + 7, {
    align: "right",
  });
...
export const generarPDFRecetas = async (recetas) => {
  ...
  recetas.forEach((receta, idx) => {
    ...
    pie(doc, pagina, totalPags, fecha);
    pagina++;
  });
